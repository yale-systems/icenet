...
class StringEncode extends Module {
  val io = IO(new Bundle {
    val in = Flipped(Decoupled(new StreamChannel(64)))
  ...
  //binary representation of wire_type 
  //binary representation of field_num 
  val wire_type = 2.U
  val field_num = 1.U
  val length = 7.U
  
  //two bytes added, one for field num and wire type 
  //second byte for length
  val num_groups = 10 
  val final_groups = Wire(Vec(num_groups, UInt(8.W)))
  
  //first byte: (field_num << 3 || wire_type)
  final_groups(0) := field_num << 3 || wire_type 
  
  //**bits coming in are not 64 bits **
  ...
  final_groups(1) := new_groups(0)
  
  //set the rest of the bits to be the same as the original from stream 
  for (i <- 2 until num_groups) {
    new_groups(i) := io.in.bits.data(8*i+7, 8*i)
  }
